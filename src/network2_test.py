"""
network2_test - 简单的网络测试工具。
1、使用 mnist_loader.py 加载 MNIST 数据集，
2、使用 network2.py 训练和测试神经网络。
"""
...
def build_arg_parser():
    ...
    parser.add_argument(
        "--mini-batch-size", type=int, default=10, help="mini-batch size for SGD"
    )
    parser.add_argument("--eta", type=float, default=3.0, help="learning rate")
    parser.add_argument(
        "--hidden-size",
        type=int,
        default=30,
    ...
    parser.add_argument(
        "--lmbda", type=float, default=0.0, help="L2 regularization strength"
    )
    # 限制样本数量，便于快速跑通与调参。
    parser.add_argument(
        "--train-limit",
...
def main():
    ...
    net = network2.Network([784, args.hidden_size, 10])
    net.SGD(
        training_data,
        args.epochs,

...
def plot_really_bad_images(images):
    """从 plot_bad_images 中挑出最糟糕的图像并绘制。"""
...
def plot_rotated_image(image):
    ...
    plt.yticks(np.array([]))
    plt.show()
    # 构造旋转图像。可用矩阵方法加速，这里采用朴素做法。
    rot_image = np.zeros((28,28))
    theta = 15*np.pi/180 # 15 度
    def to_xy(j, k):
        # 将矩阵索引转换为坐标系 (x, y)，以 (13, 14) 为原点
        return (k-13, -j+14) # x 范围：-13..14，y 范围：-13..14
    def to_jk(x, y):
        # 将 (x, y) 坐标转换回矩阵索引
        return (-y+14, x+13)
    def image_value(image, x, y):
        # 返回图像在 (x, y) 处的像素值
        # （若用闭包可避免传入 image，但此处保持简单）
        j, k = to_jk(x, y)
        return image[j, k]
    # 逐像素计算旋转后的图像：将位置反向旋转，再对邻近像素加权平均。
    # 该方法不够精确也不够快，但效果可接受。
    for j in range(28):
        for k in range(28):
            x, y = to_xy(j, k)
            ...
            # 最近的整数 x 为 x2 与 x2+1，delta_x 用于插值
            x2 = np.floor(x1)
            delta_x = x1-x2
            # y 方向同理
            y2 = np.floor(y1)
            delta_y = y1-y2
            # 越界则跳过（会漏掉边界一圈，但 MNIST 通常不会靠得太近）
            ...
            # 用经验系数缩放，补偿插值误差（可能与 theta 相关）
            rot_image[j, k] = 1.3*value
    plot_mnist_digit(rot_image)

#### 杂项
def load_data():
    """返回 MNIST 数据：(training, validation, test)。"""
